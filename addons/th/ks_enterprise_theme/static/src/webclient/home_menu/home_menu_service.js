...
export const homeMenuService = {
    ...
    start(env, { user }) {
        const state = reactive({
            hasHomeMenu: false,       // true iff the HomeMenu overlay is currently displayed
            hasBackgroundAction: false, // true iff there is an action behind the HomeMenu
            toggle,
        });
        const mutex = new Mutex(); // protect against concurrent toggling requests
        ...
        async function toggle(show) {
            return mutex.exec(async () => {
                ...
                // hack: wait for a tick to ensure url has been updated before switching again
