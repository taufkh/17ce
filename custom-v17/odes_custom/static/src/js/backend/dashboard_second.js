...
class DashboardSecond extends Component {
    static template = "odes_custom.DashboardMain_second";
     ...
     */
    async fetch_data() {
        const result = await this.rpc("/odes_custom/fetch_dashboard_data2", {
            user: this.user_id,
        ...
        if (result) {
            Object.assign(this.state, {
                ...
                lead_lost: result.lead_lost,
                active_prospect: result.active_prospect,
                active_opportunity: result.active_opportunity,
                active_report: result.active_report,
                active_pending: result.active_pending,
                opportunity_revenue: result.opportunity_revenue,
                prospect_revenue: result.prospect_revenue,
                report_revenue: result.report_revenue,
                pending_revenue: result.pending_revenue,
...
registry.category("actions").add("odes_dashboard_second", DashboardSecond);
