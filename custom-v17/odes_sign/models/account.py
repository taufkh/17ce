...
class AccountMove(models.Model):
    ...
    sale_order_no_id = fields.Many2one('sale.order',string="'Sale Quotation No'")
    # contact_ids = fields.Char(string="Contact")

    # def button_compute_invoice_taxes_by_group(self):
    ...
    def _compute_name(self):
        for move in self:
            if not move._origin.id:
                move.name = '/'
                return False

        return super(AccountMove, self)._compute_name()

    '''To override _get_payment_term_account, we have to override the whole _recompute_payment_terms_lines
    since it's a nested inside this function'''
    def _recompute_payment_terms_lines(self):
        ...
        def _get_payment_terms_account(self, payment_terms_lines):
            ...
            It's inherited so we can manually assign a default counter/payment terms account for the expenses !
            '''
            if self.is_expense and self.account_balance_id:
                return self.account_balance_id
    ...
    #def action_post(self):
        ...
        #'''We wipe anything not on invoice lines tab, the recompute the journal items tab to get the most accurate credit/debit journal'''
        #if self.is_expense:            
            #credit_lines = self.line_ids.filtered(lambda line: line.exclude_from_invoice_tab != False).unlink()
            #self._onchange_invoice_line_ids()
        #return super(AccountMove, self).action_post()

    #def recompute_line(self):
        #is expense have a unique recompute behaviour, inherit so if is_expense account.move it recomputes approriately.
        #if self.is_expense:
            #credit_lines = self.line_ids.filtered(lambda line: line.exclude_from_invoice_tab != False).unlink()
            #self._onchange_invoice_line_ids()
        #else:
            #return super(AccountMove, self).recompute_line()
...
class AccountTax(models.Model):
    ...
    #         It's inherited so we can manually assign a default counter/payment terms account for the expenses !
    #         '''
    #         res = super(AccountMove, self)._get_payment_terms_account(payment_terms_lines)
    #         if self.is_expense:
    #             return self.account_balance_id
    #         else:
    #             return res    
...
class AccountJournal(models.Model):
    ...
    payment_debit_account_id = fields.Many2one(
        ...
        domain=lambda self: "[('deprecated', '=', False), ('company_id', '=', company_id), \
                             ...
                             '|', ('user_type_id', '=', %s), ('user_type_id', '=', %s)]" % (self.env.ref('account.data_account_type_current_assets').id, self.env.ref('account.data_account_type_liquidity').id))
