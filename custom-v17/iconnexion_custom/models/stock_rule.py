...
class StockRule(models.Model):
    ...
    _inherit = 'stock.rule'
    #mark the created PO as replenish PO
    def _prepare_purchase_order(self, company_id, origins, values):
        res = super(StockRule, self)._prepare_purchase_order(company_id, origins, values)
        # res['is_replenish_po'] = True
        value = values[0]
        if value.get('related_so', False):
            res['icon_related_so_ids'] = value.get('related_so')
        return res

    #override original run_buy function so the related SO can be updated
    #when replenishing
    ...
    def _run_buy(self, procurements):
        ...
        for domain, procurements_rules in procurements_by_po_domain.items():
            ...
            if not po:
                ...
                if procurements[0].values.get('related_so', False):
                    po.write({'icon_related_so_ids': procurements[0].values.get('related_so')})
