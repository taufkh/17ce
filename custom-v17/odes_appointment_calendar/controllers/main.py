...
class WebsiteCalendar2(WebsiteCalendar):
    @http.route([
        '/calendar',
        '/calendar/<model("calendar.appointment.type"):appointment_type>',
        '/appointment',
        '/appointment/<model("calendar.appointment.type"):appointment_type>',
    ...
    @http.route(['/calendar/get_appointment_info','/appointment/get_appointment_info'], type='json', auth="public", methods=['POST'], website=True)
    ...
    @http.route(['/calendar/<model("calendar.appointment.type"):appointment_type>/appointment',
        '/appointment/<model("calendar.appointment.type"):appointment_type>/appointment'], type='http', auth="public", website=True, sitemap=True)
    ...
    @http.route(['/calendar/<model("calendar.appointment.type"):appointment_type>/info','/appointment/<model("calendar.appointment.type"):appointment_type>/info'], type='http', auth="public", website=True, sitemap=True)
    ...
    @http.route(['/calendar/<model("calendar.appointment.type"):appointment_type>/submit',
        '/appointment/<model("calendar.appointment.type"):appointment_type>/submit'], type='http', auth="public", website=True, methods=["POST"])
    def calendar_appointment_submit(self, appointment_type, datetime_str, employee_id, name, phone, email, country_id=False, **kwargs):
        ...
        if Employee.user_id and Employee.user_id.partner_id:
            if not Employee.user_id.partner_id.calendar_verify_availability(date_start, date_end):
                return request.redirect('/appointment/%s/appointment?failed=employee' % appointment_type.id)
        ...
        if Partner:
            if not Partner.calendar_verify_availability(date_start, date_end):
                return request.redirect('/appointment/%s/appointment?failed=partner' % appointment_type.id)
        ...
        return request.redirect('/appointment/view/' + event.access_token + '?message=new')

    @http.route(['/calendar/view/<string:access_token>',
        '/appointment/view/<string:access_token>'], type='http', auth="public", website=True)
    ...
    @http.route(['/calendar/cancel/<string:access_token>',
        '/appointment/cancel/<string:access_token>'], type='http', auth="public", website=True)
    def calendar_appointment_cancel(self, access_token, **kwargs):
        ...
        if fields.Datetime.from_string(event.allday and event.start_date or event.start) < datetime.now() + relativedelta(hours=event.appointment_type_id.min_cancellation_hours):
            return request.redirect('/appointment/view/' + access_token + '?message=no-cancel')
        event.unlink()
        return request.redirect('/appointment?message=cancel')
