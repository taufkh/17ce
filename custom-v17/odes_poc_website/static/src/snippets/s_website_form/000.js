...
legacyDefine('website_form.s_website_form', function (require) {
    'use strict';
...
const PublicWidgetBase = publicWidget.Widget;

    var _t = core._t;
    var qweb = core.qweb;

    publicWidget.registry.s_website_form = PublicWidgetBase.extend({
        ...
        send: async function (e) {
            ...
            var $captcha =  this.$target.find('#html_element_captcha');

            var is_continue_submit = true;
            if($captcha.length != 0){
                console.log('Captha::',grecaptcha.getResponse());
                if(grecaptcha.getResponse() == "") { 
                    is_continue_submit = false;
                    console.log('Captha is not checked:',grecaptcha.getResponse());
                    // alert("You can't proceed!");
                }else{
                    is_continue_submit = true;
                }
            }
            if(is_continue_submit == true){
