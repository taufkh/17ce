...
class OdesCrmRequirement(models.Model):
    ...
    def action_reminder_requirement_deadline(self):
        today = date.today()
        # requirements = self.env['odes.crm.requirement'].search([('state', 'not in', ['pm_confirmed', 'done']), ('date_deadline', '!=', False), ('is_sent_deadline_exceeded_reminder', '=', False)])
        requirements = self.env['odes.crm.requirement'].browse(319)
