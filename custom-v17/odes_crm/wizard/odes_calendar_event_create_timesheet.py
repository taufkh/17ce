# -*- coding: utf-8 -*-
# Part of Odoo. See LICENSE file for full copyright and licensing details.

from odoo import api, fields, models
from datetime import datetime


class MeetingCreateTimesheet(models.TransientModel):
    _name = 'calendar.event.create.timesheet'
    _description = "Create Timesheet from meeting"

    _sql_constraints = [('time_positive', 'CHECK(time_spent > 0)', 'The timesheet\'s time must be positive' )]

    time_spent = fields.Float('Time', digits=(16, 2))
    description = fields.Char('Description')
    # task_id = fields.Many2one(
    #     'project.task', "Task", required=True,
    #     default=lambda self: self.env.context.get('active_id', None),
    #     help="Task for which we are creating a sales order",
    # )
    calendar_id = fields.Many2one(
        'calendar.event', 'Meeting', required=True,
        default=lambda self: self.env.context.get('active_id', None),
        help="Task for which we are creating a sales order",
    )

    def save_timesheet(self):
        values = {
            'calendar_id': self.calendar_id.id,
            'project_id': self.calendar_id.requirement_id.project_id.id,
            'date': fields.Date.context_today(self),
            'name': self.description,
            'user_id': self.env.uid,
            'unit_amount': self.time_spent,
        }
        self.calendar_id.user_timer_id.unlink()
        return self.env['account.analytic.line'].create(values)
